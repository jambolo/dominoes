...
//!
//! This module defines the `Action` and `History` structs for tracking player actions and game history in a dominoes game. Actions
//! represent the individual moves made by players, including drawing and playing tiles. The `History` struct maintains a
//! chronological record of all actions taken during a game, allowing for game replay, analysis, and undo functionality.
//!
//! # Examples
//! ```rust
//! use player::{Action, History};
//! use rules::Tile;
//!
//! // Create a new game history
//! let mut history = History::new();
//!
//! // Player 0 draws a tile and plays a tile
//! history.add_action(Action::new(0, Some(Tile::from((1, 2))), Some(Tile::from((3, 4)))));
//!
//! // Player 1 passes their turn
//! history.add_action(Action::pass(1));
//!
//! // Get the last action from the history
//! let last_action = history.get_last_action();
//! ```
...
/// 
/// // Player plays a tile
/// let play_action = Action::play(1, Tile::from((3, 4)));
/// 
...
impl Action {
    ...
    /// let action = Action::new(0, Some(drawn_tile), Some(played_tile));
    /// 
    /// // Action where player only draws
    /// let action = Action::new(0, Some(drawn_tile), None);
    /// ```
    pub fn new(player_id: u8, tile_drawn: Option<Tile>, tile_played: Option<Tile>) -> Self {
        Self {
            player_id,
            tile_drawn,
            tile_played,
    ...
    /// let action = Action::play(0, tile);
    /// assert_eq!(action.tile_drawn, None);
    /// assert_eq!(action.tile_played, Some(tile));
    /// ```
    pub fn play(player_id: u8, tile: Tile) -> Self {
        Self {
            player_id,
            tile_drawn: None,
            tile_played: Some(tile),
        }
    }
...
impl Display for Action {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(tile) = &self.tile_played {
            write!(f, "play {tile}")?;
        }
...
mod tests {
    ...
    fn test_action_new() {
        ...
        let action = Action::new(1, Some(tile1), Some(tile2));
        
        assert_eq!(action.player_id, 1);
        assert_eq!(action.tile_drawn, Some(tile1));
        assert_eq!(action.tile_played, Some(tile2));
        ...
        let action3 = Action::new(0, None, Some(tile2));
        assert_eq!(action3.tile_drawn, None);
        assert_eq!(action3.tile_played, Some(tile2));
    }
    ...
    fn test_action_clone() {
        ...
        let action1 = Action::play(1, tile);
    ...
    fn test_history_get_player_actions() {
        ...
        history.add_action(Action::play(0, tile1));
        ...
        assert_eq!(player0_actions[0].tile_played, Some(tile1));
    ...
    fn test_history_get_actions_after_player() {
        ...
        history.add_action(Action::play(0, tile1));  // index 0
        history.add_action(Action::draw(1, tile2));   // index 1
        history.add_action(Action::pass(0));              // index 2
        history.add_action(Action::play(1, tile3));  // index 3
    ...
    fn test_action_equality() {
        let tile = Tile::from((2, 3));
        let action1 = Action::play(0, tile);
        let action2 = Action::play(0, tile);
        let action3 = Action::play(1, tile);
    ...
    fn test_action_with_different_tiles() {
        ...
        for tile in tiles {
            ...
            let play_action = Action::play(1, tile);
            assert_eq!(play_action.tile_played, Some(tile));
        }
    }
    ...
    fn test_history_add_action_order() {
        ...
        let action1 = Action::play(0, tile1);
    ...
    fn test_history_mixed_action_types() {
        ...
        history.add_action(Action::play(1, tile));
        history.add_action(Action::pass(0));
        history.add_action(Action::new(1, Some(tile), Some(tile)));
    ...
    fn test_history_get_last_action_persistence() {
        ...
        history.add_action(Action::play(0, tile1));
        assert_eq!(history.get_last_action().unwrap().tile_played, Some(tile1));
