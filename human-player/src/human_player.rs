...
impl<'a> HumanPlayer<'a> {
    ...
    fn get_player_input(&self, game_state: &DominoesState) -> (Tile, Option<u8>) {
        loop {
            // Get tile selection
    ...
    fn display_hand(&self) {
        println!("Your hand:  {}", self.hand.tiles().iter().enumerate()
    ...
    fn display_open_ends(&self, game_state: &DominoesState) {
        // Create a vector containing indexes of open ends whose count is greater than 0
        let open_ends: Vec<u8> = game_state.layout.end_counts.iter()
            .enumerate()
            .filter_map(|(end, &count)| if count > 0 { Some(end as u8) } else { None })
            .collect();
        println!("Open ends: {}", open_ends.iter()
            .map(|&e| {
                let count = game_state.layout.open_count(e);
                if count > 1 {
                    format!("{e}x{count}")
                } else {
                    format!("{e}")
    ...
    fn draw_starting_hand(&mut self, game_state: &mut DominoesState) {
        // Draw the starting hand size number of tiles from the boneyard
        let hand_size = self.configuration.starting_hand_size;
        for _ in 0..hand_size {
            let tile = game_state.draw_tile().expect("Failed to draw tile during setup");
...
impl<'a> Player for HumanPlayer<'a> {
    ...
    fn set_up(&mut self, game_state: &mut DominoesState) {
        // Draw the starting hand size number of tiles from the boneyard
        self.draw_starting_hand(game_state);
    }
    ...
    fn my_turn(&mut self, game_state: &DominoesState) -> (Action, DominoesState) {
        ...
        self.display_hand();
        let (mut tile, mut end) = self.get_player_input(game_state);
        while !game_state.can_play_tile(&tile, end) {
            println!("Please choose a playable tile and open end.");
            (tile, end) = self.get_player_input(game_state);
        }
...
mod tests {
    ...
    fn test_human_player_implements_player_trait() {
        ...
        let (returned_action, _new_state) = player.my_turn(&game_state);
    ...
    fn test_human_player_configuration_access() {
        ...
        assert_eq!(player.configuration.starting_hand_size, configuration.starting_hand_size);
    }
    ...
    fn test_human_player_setup() {
        ...
        let configuration_blind = Configuration::new(2, rules::Variation::Blind, 6, default_starting_hand_size(2, rules::Variation::Blind));
        let mut player_blind = HumanPlayer::new(0, &configuration_blind, "Test Player");
        let mut game_state_blind = DominoesState::new(&configuration_blind);

        // Blind uses 8 tiles for 2 players
        player_blind.set_up(&mut game_state_blind);
        assert_eq!(player_blind.hand.len(), 8);
        assert_eq!(game_state_blind.boneyard.count(), 20); // 28 - 8 = 20
    }
    ...
    fn test_human_player_new_different_configurations() {
        ...
        assert_eq!(player_traditional.configuration.variation, rules::Variation::Traditional);
        ...
        assert_eq!(player_blind.configuration.variation, rules::Variation::Blind);
    ...
    fn test_human_player_setup_different_hand_sizes() {
        ...
        let mut game_state_small = DominoesState::new(&config_small);
        
        player_small.set_up(&mut game_state_small);
        assert_eq!(player_small.hand.len(), 3);
        assert_eq!(game_state_small.boneyard.count(), 25); // 28 - 3
        ...
        let mut game_state_large = DominoesState::new(&config_large);
        
        player_large.set_up(&mut game_state_large);
        assert_eq!(player_large.hand.len(), 15);
        assert_eq!(game_state_large.boneyard.count(), 13); // 28 - 15
    }
    ...
    fn test_human_player_my_turn_no_playable_tiles_empty_boneyard() {
        ...
        // Empty the boneyard
        while game_state.draw_tile().is_some() {}
        
        // Player has no tiles and no playable tiles
        assert_eq!(player.hand.len(), 0);
        assert_eq!(game_state.boneyard.count(), 0);
    ...
    fn test_human_player_player_trait_consistency() {
        ...
        let _has_playable = player.has_playable_tile(&game_state);
    ...
    fn test_human_player_setup_with_different_configurations() {
        ...
        let mut game_state_2p = DominoesState::new(&config_2p);
        player_2p.set_up(&mut game_state_2p);
        ...
        let mut game_state_4p = DominoesState::new(&config_4p);
        player_4p.set_up(&mut game_state_4p);
        ...
        let mut game_state_d9 = DominoesState::new(&config_double9);
        player_d9.set_up(&mut game_state_d9);
        assert_eq!(player_d9.hand.len(), 10);
        assert_eq!(game_state_d9.boneyard.count(), 45); // 55 - 10
    }
}
